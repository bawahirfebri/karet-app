...
def process_image_segmentation(img_path, image, patch_size=128):
  ...
  resize_image = ((image.shape[0]//patch_size)*patch_size, (image.shape[1]//patch_size)*patch_size)
  ...
  model = load_model("./model_unet_v2.h5")
...
def combine_patches(large_image, canvas, segmented_image, patch_size):
  segmented_image = segmented_image.reshape((canvas.shape[0]//patch_size), (canvas.shape[1]//patch_size), patch_size, patch_size, 1)
  ...
  for i in range((canvas.shape[0]//patch_size)):
      for j in range((canvas.shape[1]//patch_size)):
        label[i*patch_size:(i+1)*patch_size, j*patch_size:(j+1)*patch_size, :] = segmented_image[i][j]

  label_output = label[:large_image.shape[0], :large_image.shape[1], :].copy()
  ...
  print(np.unique(label_output), np.unique(label_output.copy()), np.unique(cv2.bitwise_not(label_output)))
...
#   output_gulma_dataset = driver.Create(output_gulma_path, width, height, num_channels, gdal.GDT_Byte)
#   output_non_gulma_dataset = driver.Create(output_non_gulma_path, width, height, num_channels, gdal.GDT_Byte)
...
#       output_non_gulma_band = output_non_gulma_dataset.GetRasterBand(band + 1)
#       output_non_gulma_band.WriteArray(output_non_gulma[:, :, band])

#   output_gulma_dataset.GetRasterBand(1).SetColorInterpretation(gdal.GCI_RedBand)
#   output_gulma_dataset.GetRasterBand(2).SetColorInterpretation(gdal.GCI_GreenBand)
#   output_gulma_dataset.GetRasterBand(3).SetColorInterpretation(gdal.GCI_BlueBand)
#   output_gulma_dataset.GetRasterBand(4).SetColorInterpretation(gdal.GCI_AlphaBand)

#   output_non_gulma_dataset.GetRasterBand(1).SetColorInterpretation(gdal.GCI_RedBand)
#   output_non_gulma_dataset.GetRasterBand(2).SetColorInterpretation(gdal.GCI_GreenBand)
#   output_non_gulma_dataset.GetRasterBand(3).SetColorInterpretation(gdal.GCI_BlueBand)
#   output_non_gulma_dataset.GetRasterBand(4).SetColorInterpretation(gdal.GCI_AlphaBand)
